...
1. **Stable identity is the chip UID, not the by-id path.** The `/dev/serial/by-id/*` symlink prefix flips between `usb-katapult_*` and `usb-Klipper_*` when an MCU reboots. Inventory keys USB boards by `chip_uid` and resolves the live path at flash time via `flash.resolve_usb_path`. Never store the full by-id path as the primary key. **`resolve_usb_path` matches the parsed `uid` field of each by-id name *exactly* (via `discover.USB_NAME_RE`), never as a substring.** The original substring glob (`*uid*` → `matches[0]`) cross-flashed the wrong board when one UID was a substring of another's; it also raises `AmbiguousDeviceError` if one UID maps to two different MCU families (serial collision). Don't reintroduce substring/`glob` matching or the silent `[0]` fallback.
...
11. **Every flash is previewed and confirmed per board.** `flash`, `run`, and `wizard` print a `flash_plan_table` up front, then call `flash.confirm_flash(board, firmware, assume_yes=…)` before **each** `flash_board`. `confirm_flash` renders `describe_flash` (resolved device + current mode + chip UID + firmware size + build-vs-profile offsets) and prompts `[y/N]` defaulting to **No**; declining skips that one board (soft-skip, batch continues), it does not abort. `--yes/-y` auto-approves but still prints the panel — a scripted run is never silent. This is the guardrail against the blind fleet-reflash that cross-flashed a board. Keep the confirmation in the CLI/wizard loops (not buried in `flash_board`, so direct callers and tests aren't gated); a new flash path must call `confirm_flash` itself. Don't default `--yes` on, and don't collapse the per-board prompt into a single batch-level one.
12. **`run`/`wizard` skip boards whose firmware is unchanged.** `fingerprint.py` computes a per-board fingerprint = `sha256(klipper git rev + sha256(.config))` and stores it as `last_flashed:` on the board (round-trip via `inventory.record_flash`, honoring invariant #8). `run` and `wizard` call `is_up_to_date` and skip build+flash when it matches, unless `--all` is passed. **A None fingerprint (repo isn't a git checkout, git missing, or `.config` absent) always means "cannot know → do not skip → flash" — fail safe, never skip.** The fingerprint is (rev, config), NOT a hash of the built `.bin` (we must decide before building, and Klipper binaries aren't reliably byte-reproducible). It's recomputed *after* the build in `record_flash_fingerprint` because profile auto-fixes mutate `.config`. Scope is deliberately `run`/`wizard` only — the bare `flash` command is surgical (user named it, it uses a shared prebuilt `out/klipper.bin` that may not match a board's config) and neither skips nor records. `last_flashed` is machine-managed; never hand-edit it or hash the binary instead.
